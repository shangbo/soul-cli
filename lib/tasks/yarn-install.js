...
const subTasks = {
    dist: ctx => yarn(['info', `@loftysoul/soul@${ctx.version}`, '--json']).then((result) => {
        let data;
        ...
        // loftysoul
        if (!(data.engines && data.engines.cli)) {
    ...
    download: ctx => download(ctx.tarball).then((data) => {
        ...
        return decompress(data, ctx.installPath, {
            ...
            // copy extra storage adapters, eg oss..
            let installStoragePath = path.join(ctx.installPath, 'content', 'adapters', 'storage');
            let storagePath = path.join(process.cwd(), 'content', 'adapters', 'storage');
            if (fs.existsSync(installStoragePath)){
                ctx.ui.sudo('ln -sf ' + installStoragePath + ' ' + storagePath);
            }
        }).catch((error) => {
